...
class ListTodo:
    ...
    def increase_priority(self, todo_id: str, user_id: str) -> None:
        ...
        created_date_of_todo = todo.created_at.strftime("%Y-%m-%d")

        all_todos_by_created_date = self.repository.get_by_user_id_and_date_by_priority(
            todo.user_id, created_date_of_todo
        )

        dict_priority_count = self._priority_dict_count(all_todos_by_created_date)

        max_priority = self._max_priority(all_todos_by_created_date)
        ...
        if todo.priority < max_priority or (
            ...
            raise AlreadyOnHighestPriority("Already on the highest priority")
    ...
    def decrease_priority(self, todo_id: str, user_id: str) -> None:
        ...
        if todo.priority > 0:
            ...
            raise AlreadyOnLowPriority("Already on the lowest priority")
